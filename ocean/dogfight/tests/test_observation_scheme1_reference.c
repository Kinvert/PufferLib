...
static const float DOGFIGHT3_SCHEME1_STEEP_OBS[TEST_OBS_SIZE] = {
    0.230656311f,
    -1.000000000f,
    -0.085228600f,
    -0.933333337f,
...
static void setup_steep_env(TestEnv* t) {
    setup_env(t);

    t->env.tick = 777;
    t->env.max_steps = 1800;
    t->env.player.pos = vec3(-2200.0f, 1450.0f, 4100.0f);
    t->env.player.vel = vec3(-70.0f, 240.0f, 95.0f);
    t->env.player.prev_vel = vec3(-74.0f, 235.0f, 89.0f);
    t->env.player.omega = vec3(-2.8f, 3.4f, -4.7f);
    t->env.player.ori = make_ori(2.35f, 0.92f, -1.18f);
    t->env.player.throttle = 0.92f;
    t->env.player.g_force = 5.8f;

    t->env.opponent.pos = vec3(640.0f, -980.0f, 760.0f);
    t->env.opponent.vel = vec3(230.0f, -150.0f, -120.0f);
    t->env.opponent.prev_vel = vec3(225.0f, -145.0f, -110.0f);
    t->env.opponent.omega = vec3(4.6f, -3.9f, 2.7f);
    t->env.opponent.ori = make_ori(-2.70f, -0.77f, 1.34f);
    t->env.opponent.throttle = 0.28f;
    t->env.opponent.g_force = -1.2f;
}

static int compare_observations(const char* label, TestEnv* t, const float* expected_obs) {
    compute_observations(&t->env);
    for (int i = 0; i < TEST_OBS_SIZE; i++) {
        float got = t->observations[i];
        float expected = expected_obs[i];
        if (fabsf(got - expected) > 1e-5f) {
            printf("%s: obs[%d]=%.9f expected %.9f [FAIL]\n",
...
static int test_scheme1_observations_match_dogfight3_fixtures(void) {
    TestEnv t;
    setup_env(&t);
    ...
    if (compare_observations("scheme1_reference", &t, DOGFIGHT3_SCHEME1_OBS)) return 1;

    setup_steep_env(&t);
    if (compare_observations("scheme1_steep_reference", &t, DOGFIGHT3_SCHEME1_STEEP_OBS)) return 1;

    printf("scheme1_reference: observations match Dogfight3 fixtures [OK]\n");
    return 0;
}

int main(void) {
    return test_scheme1_observations_match_dogfight3_fixtures();
}
