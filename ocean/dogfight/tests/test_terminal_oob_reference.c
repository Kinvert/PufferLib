...
typedef struct ExpectedOob {
    const char* label;
    int opponent_crashes;
    float reward;
    float opponent_reward;
    float log_episode_return;
    float log_score;
    float player_ground_hits;
    float opponent_ground_hits;
} ExpectedOob;
...
static void setup_env(TestEnv* t, int opponent_crashes) {
    memset(t, 0, sizeof(*t));
    t->env.num_agents = 1;
    ...
    float player_z = opponent_crashes ? 1800.0f : -5.0f;
    float opponent_z = opponent_crashes ? -5.0f : 1800.0f;

    t->env.player.pos = vec3(0.0f, 0.0f, player_z);
    t->env.player.vel = vec3(90.0f, 0.0f, opponent_crashes ? 0.0f : -10.0f);
    ...
    t->env.opponent.pos = vec3(260.0f, 0.0f, opponent_z);
    t->env.opponent.vel = vec3(80.0f, 0.0f, opponent_crashes ? -10.0f : 0.0f);
...
static int compare_expected(TestEnv* t, const ExpectedOob* e) {
    if (!nearly(t->rewards[0], e->reward, 1e-6f) ||
            !nearly(t->opponent_rewards[0], e->opponent_reward, 1e-6f) ||
            !nearly(t->terminals[0], 1.0f, 1e-6f)) {
        printf("%s: rewards player=%.9f opponent=%.9f terminal=%.1f [FAIL]\n",
            e->label, t->rewards[0], t->opponent_rewards[0], t->terminals[0]);
        return 1;
    }

    if (t->env.tick != 0 || t->env.death_reason != DEATH_NONE ||
            t->env.last_death_reason != DEATH_OOB || t->env.last_winner != 0 ||
            t->env.kill != 0 || t->env.opp_kill != 0 ||
            t->env.total_episodes != 1 ||
            !nearly(t->env.episode_return, 0.0f, 1e-6f)) {
        printf("%s: reset flags tick=%d death=%d last=%d winner=%d kill=%d opp=%d eps=%d [FAIL]\n",
            e->label, t->env.tick, t->env.death_reason,
            t->env.last_death_reason, t->env.last_winner,
            t->env.kill, t->env.opp_kill, t->env.total_episodes);
        return 1;
    }

    if (!nearly(t->env.log.episode_return, e->log_episode_return, 1e-6f) ||
            !nearly(t->env.log.episode_length, 1.0f, 1e-6f) ||
            !nearly(t->env.log.perf, 0.0f, 1e-6f) ||
            !nearly(t->env.log.score, e->log_score, 1e-6f) ||
            ...
            !nearly(t->env.log.player_ground_hits, e->player_ground_hits, 1e-6f) ||
            !nearly(t->env.log.opponent_ground_hits, e->opponent_ground_hits, 1e-6f) ||
        ...
        printf("%s: log return=%.9f score=%.9f base=%.9f/%.9f ground=%.9f/%.9f [FAIL]\n",
            e->label, t->env.log.episode_return, t->env.log.score,
...
static int test_terminal_oob_matches_dogfight3_fixture(void) {
    const ExpectedOob cases[] = {
        {
            .label = "player_oob",
            .opponent_crashes = 0,
            .reward = -1.0f,
            .opponent_reward = 0.25f,
            .log_episode_return = -0.012001647f,
            .log_score = -1.0f,
            .player_ground_hits = 1.0f,
            .opponent_ground_hits = 0.0f,
        },
        {
            .label = "opponent_oob",
            .opponent_crashes = 1,
            .reward = 0.25f,
            .opponent_reward = -1.0f,
            .log_episode_return = 0.004168154f,
            .log_score = 0.25f,
            .player_ground_hits = 0.0f,
            .opponent_ground_hits = 1.0f,
        },
    };

    for (int i = 0; i < 2; i++) {
        TestEnv t;
        setup_env(&t, cases[i].opponent_crashes);
        c_step(&t.env);
        if (compare_expected(&t, &cases[i])) return 1;
    }

    printf("terminal_oob: player and opponent OOB rewards, reset flags, and log counters match Dogfight3 fixture [OK]\n");
    return 0;
}

int main(void) {
    return test_terminal_oob_matches_dogfight3_fixture();
}
