...
static void setup_env(TestEnv* t) {
    ...
    t->env.player.pos = vec3(260.0f, 0.0f, 1800.0f);
    t->env.player.vel = vec3(105.0f, 0.0f, 0.0f);
    ...
    t->env.opponent.pos = vec3(0.0f, 0.0f, 1800.0f);
    t->env.opponent.vel = vec3(120.0f, 0.0f, 0.0f);
...
static int test_terminal_opponent_kill_matches_df36_fixture(void) {
    TestEnv t;
    setup_env(&t);
    c_step(&t.env);
    ...
    if (!nearly(t.rewards[0], -1.0f, 1e-6f) ||
        ...
        printf("terminal_opponent_kill: rewards player=%.9f opponent=%.9f terminal=%.1f [FAIL]\n",
    ...
    if (t.env.tick != 0 || t.env.death_reason != DEATH_NONE ||
        ...
        printf("terminal_opponent_kill: reset flags tick=%d death=%d last=%d winner=%d kill=%d opp=%d eps=%d [FAIL]\n",
            t.env.tick, t.env.death_reason, t.env.last_death_reason,
            t.env.last_winner, t.env.kill, t.env.opp_kill, t.env.total_episodes);
        return 1;
    }

    if (!nearly(t.env.log.episode_return, 0.0f, 1e-6f) ||
        ...
        printf("terminal_opponent_kill: log return=%.9f score=%.9f sp=%.9f/%.9f clean=%.9f base=%.9f/%.9f [FAIL]\n",
            t.env.log.episode_return, t.env.log.score,
            t.env.log.sp_player_kills, t.env.log.sp_opp_kills,
            t.env.log.clean_fights, t.env.log.base_stage_kills,
            t.env.log.base_stage_eps);
        return 1;
    }

    printf("terminal_opponent_kill: opponent kill rewards, reset flags, and self-play logs match df36 fixture [OK]\n");
    return 0;
}

int main(void) {
    return test_terminal_opponent_kill_matches_df36_fixture();
}
