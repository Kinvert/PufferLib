...
DOGFIGHT3_ACTION_INIT_SCALE = 0.01
...
def test_dogfight_policy_config_ports_dogfight3_decoder_init_scales():
    parser = ConfigParser()
    parser.read(repo_root() / "config" / "dogfight.ini")

    assert math.isclose(
        parser.getfloat("policy", "action_init_scale"),
        DOGFIGHT3_ACTION_INIT_SCALE,
    )
    assert math.isclose(
...
def test_dogfight_policy_config_starts_from_dogfight3_recurrent_depth():
    ...
    assert parser.getint("sweep.policy.num_layers", "mean") == 1
    assert parser.getint("sweep.policy.num_layers", "max") >= 3


def test_dogfight_sweep_can_search_native_action_head_init_scale():
    parser = ConfigParser()
    parser.read(repo_root() / "config" / "dogfight.ini")

    assert parser.get("sweep.policy.action_init_scale", "distribution") == "log_normal"
    assert parser.getfloat("sweep.policy.action_init_scale", "min") < DOGFIGHT3_ACTION_INIT_SCALE
    assert math.isclose(
        parser.getfloat("sweep.policy.action_init_scale", "mean"),
        DOGFIGHT3_ACTION_INIT_SCALE,
    )
    assert parser.getfloat("sweep.policy.action_init_scale", "max") > DOGFIGHT3_ACTION_INIT_SCALE


def test_dogfight_sweep_keeps_reference_recurrent_action_init_reachable():
    parser = ConfigParser()
    parser.read(repo_root() / "config" / "dogfight.ini")
    ...
    assert parser.getfloat("sweep.policy.action_init_scale", "max") >= (
