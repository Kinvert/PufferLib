...
def test_prepare_stages_only_the_intended_native_sweep_dimensions(tmp_path):
    ...
    assert staged_dogfight["base"]["checkpoint_interval"] == "384"
    assert staged_dogfight["base"]["wandb_project"] == "df42"
    ...
    assert manifest["wandb_project"] == "df42"
...
def test_default_profile_is_a_bounded_screening_run(tmp_path):
    ...
    assert staged["train"]["total_timesteps"] == "671_088_640"
    assert staged["env"]["native_spawn_total_steps"] == "671_088_640"
    spawn_sweep = staged["sweep.env.native_spawn_total_steps"]
    assert spawn_sweep["min"] == "402_653_184"
    assert spawn_sweep["max"] == "805_306_368"
    assert staged["env"]["native_acquisition_steps"] == "134217728"
    assert staged["base"]["checkpoint_interval"] == "384"
