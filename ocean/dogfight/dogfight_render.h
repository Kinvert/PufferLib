...
static const char* OBS_LABELS_OPPONENT_AWARE[26] = {
    "fwd_spd", "sideslip", "climb", "roll_r", "pitch_r", "yaw_r",
    "aoa", "altitude", "g_force", "energy",
    "up_x", "up_y", "up_z",
    "tgt_az", "tgt_el", "range", "closure",
...
void draw_obs_monitor(Dogfight *env) {
    int start_x = GetScreenWidth() - 350;
    int start_y = 10;
    int row_height = 18;
    ...
    switch (env->obs_scheme) {
        ...
        case OBS_OPPONENT_AWARE:
            labels = OBS_LABELS_OPPONENT_AWARE;
            break;
        default:
            labels = OBS_LABELS_PILOT;
            num_obs = 22;
...
void c_render(Dogfight *env) {
    if (env->client == NULL) {
        ...
        if (!env->client->model_loaded) {
            fprintf(stderr, "Failed to load ocean/dogfight/p40.glb\n");
            exit(1);
        }
    }
    ...
    if (env->tick < 100 && env->last_death_reason != DEATH_NONE) {
        ...
        if (result_text) {
            ...
            DrawText(result_text, (GetScreenWidth() - text_width) / 2, 450, 50, result_color);
        }
    }
