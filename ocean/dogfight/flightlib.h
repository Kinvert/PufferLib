...
#ifdef __cplusplus
static thread_local unsigned int* dogfight_active_rng = NULL;
#else
static _Thread_local unsigned int* dogfight_active_rng = NULL;
#endif

static inline void dogfight_bind_rng(unsigned int* rng) {
    dogfight_active_rng = rng;
}

static inline unsigned int dogfight_rand(void) {
    if (dogfight_active_rng != NULL) {
        return (unsigned int)rand_r(dogfight_active_rng);
    }
    return (unsigned int)rand();
}

static inline float rndf(float a, float b) {
    return a + ((float)dogfight_rand() / (float)RAND_MAX) * (b - a);
}
...
static inline FlightParams default_flight_params(void) {
    return (FlightParams){
        ...
        .c_d0 = C_D0, .k = INDUCED_DRAG_K, .c_l_alpha = C_L_ALPHA, .c_l_max = C_L_MAX, .rho = RHO,
...
static inline void randomize_flight_params(FlightParams* params, float dr) {
    ...
    params->k = INDUCED_DRAG_K * rndf(1.0f - dr, 1.0f + dr);
...
static inline void compute_derivatives(Plane* state, float* actions, float dt, StateDerivative* deriv) {
    ...
    float induced_term = INDUCED_DRAG_K * C_L * C_L;
