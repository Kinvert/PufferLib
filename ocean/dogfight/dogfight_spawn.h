...
static inline float stage9_bank_deg_for_target(float target) {
    if (target < 8.6f) return 0.0f;
    if (target < 8.7f) return 5.0f;
    if (target < 8.8f) return 10.0f;
    if (target < 8.9f) return 15.0f;
    return 30.0f;
}

static inline float resolve_side_bank_deg(Dogfight* env, const StageConfig* cfg) {
    if (env->stage != CURRICULUM_SIDE_MANEUVERING) {
        return (float)cfg->bank;
    }
    if (env->stage9_bank_deg >= 0.0f) {
        return env->stage9_bank_deg;
    }
    return stage9_bank_deg_for_target(env->curriculum_target);
}
...
static void spawn_side(Dogfight *env, Vec3 player_pos, Vec3 player_vel) {
    ...
    float bank_deg = resolve_side_bank_deg(env, cfg);
...
void spawn_by_curriculum(Dogfight *env, Vec3 player_pos, Vec3 player_vel) {
    ...
    if (env->stage < CURRICULUM_COUNT) {
        ...
        // Use per-stage max_steps for side-far and later stages
