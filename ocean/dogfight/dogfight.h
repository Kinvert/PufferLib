...
// max_steps documents each stage's intended cap. Early stages inherit the
// configured Python cap; later pursuit stages may override it locally.
...
typedef struct Log {
    ...
    float base_stage_ground;        // Ground-crash terminations at current mastery stage
    ...
    float base_stage_action_sat_trigger;
    float base_stage_signed_bias_elevator;
    float base_stage_signed_bias_aileron;
    float base_stage_signed_bias_rudder;
    float base_stage_side_standard_eps;
    float base_stage_side_standard_kills;
    float base_stage_side_standard_ground;
    float base_stage_side_standard_timeouts;
    float base_stage_side_standard_episode_length;
    float base_stage_side_standard_action_saturation;
    float base_stage_side_standard_signed_bias;
    float base_stage_side_standard_action_sat_elevator;
    float base_stage_side_standard_action_sat_aileron;
    float base_stage_side_standard_action_sat_rudder;
    float base_stage_side_standard_action_sat_trigger;
    float base_stage_side_standard_signed_bias_elevator;
    float base_stage_side_standard_signed_bias_aileron;
    float base_stage_side_standard_signed_bias_rudder;
    float base_stage_side_energy_eps;
    float base_stage_side_energy_kills;
    float base_stage_side_energy_ground;
    float base_stage_side_energy_timeouts;
    float base_stage_side_energy_episode_length;
    float base_stage_side_energy_action_saturation;
    float base_stage_side_energy_signed_bias;
    float base_stage_side_energy_action_sat_elevator;
    float base_stage_side_energy_action_sat_aileron;
    float base_stage_side_energy_action_sat_rudder;
    float base_stage_side_energy_action_sat_trigger;
    float base_stage_side_energy_signed_bias_elevator;
    float base_stage_side_energy_signed_bias_aileron;
    float base_stage_side_energy_signed_bias_rudder;
...
typedef enum SideSpawnVariant {
    SIDE_SPAWN_NONE = 0,
    SIDE_SPAWN_STANDARD = 1,
    SIDE_SPAWN_ENERGY = 2,
} SideSpawnVariant;
...
typedef struct RuntimeConfig {
    ...
    float stage9_bank_deg;
    float side_energy_spawn_prob;
} RuntimeConfig;
...
typedef struct State {
    ...
    int curriculum_randomize;
    int total_episodes;
    CurriculumStage stage;
    float curriculum_target;
    int is_initialized;
    float total_aileron_usage;
    float elevator_bias;
    float aileron_bias;
    float rudder_bias;
    float episode_control_rate;
    int low_altitude_variant;
    SideSpawnVariant side_spawn_variant;
    ...
    int last_winner;
    float last_opp_actions[5];
    float opponent_actions_override[5];
    int use_opponent_override;
    float selfplay_prob;
    int head_on_lockout;
    float prev_rel_dot;
    int eval_spawn_mode;
    ...
    float prev_rudder;
    long global_step;
    long debug_trigger_step;
    int selfplay_active;
    int opponent_recovery_active;
    int opponent_recovery_tick_start;
    float recovery_altitude_threshold;
    float recovery_trigger_prob;
    float recovery_speed_threshold;
    float recovery_bank_deg;
    unsigned int recovery_rng_state;
    int opponent_above_recovery_threshold;
    int guided_climb_active;
    int guided_climb_ticks_remaining;
    float guided_climb_elevator;
    float vertical_spawn_prob;
    int vertical_level;
    int vertical_spawn_used;
    float stage9_bank_deg;
    float side_energy_spawn_prob;
...
typedef struct Dogfight {
    ...
    float elevator_bias;        // Cumulative signed elevator input
    float aileron_bias;         // Cumulative signed aileron (for directional penalty)
    float rudder_bias;          // Cumulative signed rudder input
    ...
    SideSpawnVariant side_spawn_variant; // Stage 6-9 side-spawn subtype for diagnostics
    float episode_action_abs_elevator;
    float episode_action_abs_aileron;
    float episode_action_abs_rudder;
    float episode_action_abs_trigger;
    ...
    float stage9_bank_deg;        // Stage 9 side-maneuver bank override; <0 uses target ladder
    float side_energy_spawn_prob; // Probability of the energy-building side-spawn variant
...
static inline void apply_runtime_config(Dogfight* env, const RuntimeConfig* cfg) {
    ...
    env->stage9_bank_deg = cfg->stage9_bank_deg;
    env->side_energy_spawn_prob = clampf(cfg->side_energy_spawn_prob, 0.0f, 1.0f);
}

#include "dogfight_observations.h"

static inline void dogfight_state_capture(Dogfight* env) {
    State* s = &env->state;
    s->rng = env->rng;
    s->tick = env->tick;
    s->max_steps = env->max_steps;
    s->configured_max_steps = env->configured_max_steps;
    s->episode_return = env->episode_return;
    s->player = env->player;
    s->opponent = env->opponent;
    s->gun_cone_angle = env->gun_cone_angle;
    s->cos_gun_cone = env->cos_gun_cone;
    s->opponent_ap = env->opponent_ap;
    s->opponent_ace = env->opponent_ace;
    s->obs_scheme = env->obs_scheme;
    s->obs_size = env->obs_size;
    s->opponent_obs_scheme = env->opponent_obs_scheme;
    s->opponent_obs_size = env->opponent_obs_size;
    s->rcfg = env->rcfg;
    s->kill = env->kill;
    s->opp_kill = env->opp_kill;
    s->episode_shots_fired = env->episode_shots_fired;
    s->curriculum_enabled = env->curriculum_enabled;
    s->curriculum_randomize = env->curriculum_randomize;
    s->total_episodes = env->total_episodes;
    s->stage = env->stage;
    s->curriculum_target = env->curriculum_target;
    s->is_initialized = env->is_initialized;
    s->total_aileron_usage = env->total_aileron_usage;
    s->elevator_bias = env->elevator_bias;
    s->aileron_bias = env->aileron_bias;
    s->rudder_bias = env->rudder_bias;
    s->episode_control_rate = env->episode_control_rate;
    s->low_altitude_variant = env->low_altitude_variant;
    s->side_spawn_variant = env->side_spawn_variant;
    s->episode_action_abs_elevator = env->episode_action_abs_elevator;
    s->episode_action_abs_aileron = env->episode_action_abs_aileron;
    s->episode_action_abs_rudder = env->episode_action_abs_rudder;
    s->episode_action_abs_trigger = env->episode_action_abs_trigger;
    s->episode_action_sat_elevator = env->episode_action_sat_elevator;
    s->episode_action_sat_aileron = env->episode_action_sat_aileron;
    s->episode_action_sat_rudder = env->episode_action_sat_rudder;
    s->episode_action_sat_trigger = env->episode_action_sat_trigger;
    s->sum_r_closing = env->sum_r_closing;
    s->sum_r_speed = env->sum_r_speed;
    s->sum_r_neg_g = env->sum_r_neg_g;
    s->sum_r_rudder = env->sum_r_rudder;
    s->sum_r_aim = env->sum_r_aim;
    s->sum_r_rate = env->sum_r_rate;
    s->sum_r_altitude = env->sum_r_altitude;
    s->sum_r_time = env->sum_r_time;
    s->sum_r_player_energy = env->sum_r_player_energy;
    s->sum_r_energy_adv = env->sum_r_energy_adv;
    s->r_closing = env->r_closing;
    s->r_aim = env->r_aim;
    s->r_neg_g = env->r_neg_g;
    s->r_stall = env->r_stall;
    s->r_rudder = env->r_rudder;
    s->r_rate = env->r_rate;
    s->r_altitude = env->r_altitude;
    s->r_time = env->r_time;
    s->r_player_energy = env->r_player_energy;
    s->r_energy_adv = env->r_energy_adv;
    s->best_aim_angle = env->best_aim_angle;
    s->ticks_in_cone = env->ticks_in_cone;
    s->closest_dist = env->closest_dist;
    s->max_g = env->max_g;
    s->min_g = env->min_g;
    s->max_bank = env->max_bank;
    s->max_pitch = env->max_pitch;
    s->min_speed = env->min_speed;
    s->max_speed = env->max_speed;
    s->min_alt = env->min_alt;
    s->max_alt = env->max_alt;
    s->sum_throttle = env->sum_throttle;
    s->trigger_pulls = env->trigger_pulls;
    s->prev_trigger = env->prev_trigger;
    s->death_reason = env->death_reason;
    s->last_death_reason = env->last_death_reason;
    s->last_winner = env->last_winner;
    memcpy(s->last_opp_actions, env->last_opp_actions, sizeof(s->last_opp_actions));
    memcpy(s->opponent_actions_override, env->opponent_actions_override, sizeof(s->opponent_actions_override));
    s->use_opponent_override = env->use_opponent_override;
    s->selfplay_prob = env->selfplay_prob;
    s->head_on_lockout = env->head_on_lockout;
    s->prev_rel_dot = env->prev_rel_dot;
    s->eval_spawn_mode = env->eval_spawn_mode;
    s->prev_elevator = env->prev_elevator;
    s->prev_aileron = env->prev_aileron;
    s->prev_rudder = env->prev_rudder;
    s->global_step = env->global_step;
    s->debug_trigger_step = env->debug_trigger_step;
    s->selfplay_active = env->selfplay_active;
    s->opponent_recovery_active = env->opponent_recovery_active;
    s->opponent_recovery_tick_start = env->opponent_recovery_tick_start;
    s->recovery_altitude_threshold = env->recovery_altitude_threshold;
    s->recovery_trigger_prob = env->recovery_trigger_prob;
    s->recovery_speed_threshold = env->recovery_speed_threshold;
    s->recovery_bank_deg = env->recovery_bank_deg;
    s->recovery_rng_state = env->recovery_rng_state;
    s->opponent_above_recovery_threshold = env->opponent_above_recovery_threshold;
    s->guided_climb_active = env->guided_climb_active;
    s->guided_climb_ticks_remaining = env->guided_climb_ticks_remaining;
    s->guided_climb_elevator = env->guided_climb_elevator;
    s->vertical_spawn_prob = env->vertical_spawn_prob;
    s->vertical_level = env->vertical_level;
    s->vertical_spawn_used = env->vertical_spawn_used;
    s->stage9_bank_deg = env->stage9_bank_deg;
    s->side_energy_spawn_prob = env->side_energy_spawn_prob;
    s->prev_player_target_az = env->prev_player_target_az;
    s->prev_player_target_el = env->prev_player_target_el;
    s->prev_player_aspect = env->prev_player_aspect;
    s->prev_player_eadv = env->prev_player_eadv;
    s->prev_opp_target_az = env->prev_opp_target_az;
    s->prev_opp_target_el = env->prev_opp_target_el;
    s->prev_opp_aspect = env->prev_opp_aspect;
    s->prev_opp_eadv = env->prev_opp_eadv;
    s->flight_params = env->flight_params;
    s->domain_randomization = env->domain_randomization;
}

static inline void dogfight_state_restore(Dogfight* env) {
    State* s = &env->state;
    env->rng = s->rng;
    env->tick = s->tick;
    env->max_steps = s->max_steps;
    env->configured_max_steps = s->configured_max_steps;
    env->episode_return = s->episode_return;
    env->player = s->player;
    env->opponent = s->opponent;
    env->gun_cone_angle = s->gun_cone_angle;
    env->cos_gun_cone = s->cos_gun_cone;
    env->opponent_ap = s->opponent_ap;
    env->opponent_ace = s->opponent_ace;
    env->obs_scheme = s->obs_scheme;
    env->obs_size = s->obs_size;
    env->opponent_obs_scheme = s->opponent_obs_scheme;
    env->opponent_obs_size = s->opponent_obs_size;
    env->rcfg = s->rcfg;
    env->kill = s->kill;
    env->opp_kill = s->opp_kill;
    env->episode_shots_fired = s->episode_shots_fired;
    env->curriculum_enabled = s->curriculum_enabled;
    env->curriculum_randomize = s->curriculum_randomize;
    env->total_episodes = s->total_episodes;
    env->stage = s->stage;
    env->curriculum_target = s->curriculum_target;
    env->is_initialized = s->is_initialized;
    env->total_aileron_usage = s->total_aileron_usage;
    env->elevator_bias = s->elevator_bias;
    env->aileron_bias = s->aileron_bias;
    env->rudder_bias = s->rudder_bias;
    env->episode_control_rate = s->episode_control_rate;
    env->low_altitude_variant = s->low_altitude_variant;
    env->side_spawn_variant = s->side_spawn_variant;
    env->episode_action_abs_elevator = s->episode_action_abs_elevator;
    env->episode_action_abs_aileron = s->episode_action_abs_aileron;
    env->episode_action_abs_rudder = s->episode_action_abs_rudder;
    env->episode_action_abs_trigger = s->episode_action_abs_trigger;
    env->episode_action_sat_elevator = s->episode_action_sat_elevator;
    env->episode_action_sat_aileron = s->episode_action_sat_aileron;
    env->episode_action_sat_rudder = s->episode_action_sat_rudder;
    env->episode_action_sat_trigger = s->episode_action_sat_trigger;
    env->sum_r_closing = s->sum_r_closing;
    env->sum_r_speed = s->sum_r_speed;
    env->sum_r_neg_g = s->sum_r_neg_g;
    env->sum_r_rudder = s->sum_r_rudder;
    env->sum_r_aim = s->sum_r_aim;
    env->sum_r_rate = s->sum_r_rate;
    env->sum_r_altitude = s->sum_r_altitude;
    env->sum_r_time = s->sum_r_time;
    env->sum_r_player_energy = s->sum_r_player_energy;
    env->sum_r_energy_adv = s->sum_r_energy_adv;
    env->r_closing = s->r_closing;
    env->r_aim = s->r_aim;
    env->r_neg_g = s->r_neg_g;
    env->r_stall = s->r_stall;
    env->r_rudder = s->r_rudder;
    env->r_rate = s->r_rate;
    env->r_altitude = s->r_altitude;
    env->r_time = s->r_time;
    env->r_player_energy = s->r_player_energy;
    env->r_energy_adv = s->r_energy_adv;
    env->best_aim_angle = s->best_aim_angle;
    env->ticks_in_cone = s->ticks_in_cone;
    env->closest_dist = s->closest_dist;
    env->max_g = s->max_g;
    env->min_g = s->min_g;
    env->max_bank = s->max_bank;
    env->max_pitch = s->max_pitch;
    env->min_speed = s->min_speed;
    env->max_speed = s->max_speed;
    env->min_alt = s->min_alt;
    env->max_alt = s->max_alt;
    env->sum_throttle = s->sum_throttle;
    env->trigger_pulls = s->trigger_pulls;
    env->prev_trigger = s->prev_trigger;
    env->death_reason = s->death_reason;
    env->last_death_reason = s->last_death_reason;
    env->last_winner = s->last_winner;
    memcpy(env->last_opp_actions, s->last_opp_actions, sizeof(env->last_opp_actions));
    memcpy(env->opponent_actions_override, s->opponent_actions_override, sizeof(env->opponent_actions_override));
    env->use_opponent_override = s->use_opponent_override;
    env->selfplay_prob = s->selfplay_prob;
    env->head_on_lockout = s->head_on_lockout;
    env->prev_rel_dot = s->prev_rel_dot;
    env->eval_spawn_mode = s->eval_spawn_mode;
    env->prev_elevator = s->prev_elevator;
    env->prev_aileron = s->prev_aileron;
    env->prev_rudder = s->prev_rudder;
    env->global_step = s->global_step;
    env->debug_trigger_step = s->debug_trigger_step;
    env->selfplay_active = s->selfplay_active;
    env->opponent_recovery_active = s->opponent_recovery_active;
    env->opponent_recovery_tick_start = s->opponent_recovery_tick_start;
    env->recovery_altitude_threshold = s->recovery_altitude_threshold;
    env->recovery_trigger_prob = s->recovery_trigger_prob;
    env->recovery_speed_threshold = s->recovery_speed_threshold;
    env->recovery_bank_deg = s->recovery_bank_deg;
    env->recovery_rng_state = s->recovery_rng_state;
    env->opponent_above_recovery_threshold = s->opponent_above_recovery_threshold;
    env->guided_climb_active = s->guided_climb_active;
    env->guided_climb_ticks_remaining = s->guided_climb_ticks_remaining;
    env->guided_climb_elevator = s->guided_climb_elevator;
    env->vertical_spawn_prob = s->vertical_spawn_prob;
    env->vertical_level = s->vertical_level;
    env->vertical_spawn_used = s->vertical_spawn_used;
    env->stage9_bank_deg = s->stage9_bank_deg;
    env->side_energy_spawn_prob = s->side_energy_spawn_prob;
    env->prev_player_target_az = s->prev_player_target_az;
    env->prev_player_target_el = s->prev_player_target_el;
    env->prev_player_aspect = s->prev_player_aspect;
    env->prev_player_eadv = s->prev_player_eadv;
    env->prev_opp_target_az = s->prev_opp_target_az;
    env->prev_opp_target_el = s->prev_opp_target_el;
    env->prev_opp_aspect = s->prev_opp_aspect;
    env->prev_opp_eadv = s->prev_opp_eadv;
    env->flight_params = s->flight_params;
    env->domain_randomization = s->domain_randomization;
}

static inline void dogfight_state_refresh(Dogfight* env) {
    dogfight_state_restore(env);
    if (env->rewards != NULL) env->rewards[0] = 0.0f;
    if (env->terminals != NULL) env->terminals[0] = 0.0f;
    if (env->opponent_rewards != NULL) env->opponent_rewards[0] = 0.0f;
...
void init(Dogfight *env, int obs_scheme, RewardConfig *rcfg, int curriculum_enabled, int curriculum_randomize, int env_num) {
    ...
    env->elevator_bias = 0.0f;
    env->rudder_bias = 0.0f;
    env->low_altitude_variant = 0;
    env->side_spawn_variant = SIDE_SPAWN_NONE;
...
static void add_side_variant_log(Dogfight* env, int ground_termination,
        float episode_ticks, float surface_saturation) {
    float* eps = NULL;
    float* kills = NULL;
    float* ground = NULL;
    float* timeouts = NULL;
    float* episode_length = NULL;
    float* action_saturation = NULL;
    float* signed_bias = NULL;
    float* action_sat_elevator = NULL;
    float* action_sat_aileron = NULL;
    float* action_sat_rudder = NULL;
    float* action_sat_trigger = NULL;
    float* signed_bias_elevator = NULL;
    float* signed_bias_aileron = NULL;
    float* signed_bias_rudder = NULL;

    if (env->side_spawn_variant == SIDE_SPAWN_STANDARD) {
        eps = &env->log.base_stage_side_standard_eps;
        kills = &env->log.base_stage_side_standard_kills;
        ground = &env->log.base_stage_side_standard_ground;
        timeouts = &env->log.base_stage_side_standard_timeouts;
        episode_length = &env->log.base_stage_side_standard_episode_length;
        action_saturation = &env->log.base_stage_side_standard_action_saturation;
        signed_bias = &env->log.base_stage_side_standard_signed_bias;
        action_sat_elevator = &env->log.base_stage_side_standard_action_sat_elevator;
        action_sat_aileron = &env->log.base_stage_side_standard_action_sat_aileron;
        action_sat_rudder = &env->log.base_stage_side_standard_action_sat_rudder;
        action_sat_trigger = &env->log.base_stage_side_standard_action_sat_trigger;
        signed_bias_elevator = &env->log.base_stage_side_standard_signed_bias_elevator;
        signed_bias_aileron = &env->log.base_stage_side_standard_signed_bias_aileron;
        signed_bias_rudder = &env->log.base_stage_side_standard_signed_bias_rudder;
    } else if (env->side_spawn_variant == SIDE_SPAWN_ENERGY) {
        eps = &env->log.base_stage_side_energy_eps;
        kills = &env->log.base_stage_side_energy_kills;
        ground = &env->log.base_stage_side_energy_ground;
        timeouts = &env->log.base_stage_side_energy_timeouts;
        episode_length = &env->log.base_stage_side_energy_episode_length;
        action_saturation = &env->log.base_stage_side_energy_action_saturation;
        signed_bias = &env->log.base_stage_side_energy_signed_bias;
        action_sat_elevator = &env->log.base_stage_side_energy_action_sat_elevator;
        action_sat_aileron = &env->log.base_stage_side_energy_action_sat_aileron;
        action_sat_rudder = &env->log.base_stage_side_energy_action_sat_rudder;
        action_sat_trigger = &env->log.base_stage_side_energy_action_sat_trigger;
        signed_bias_elevator = &env->log.base_stage_side_energy_signed_bias_elevator;
        signed_bias_aileron = &env->log.base_stage_side_energy_signed_bias_aileron;
        signed_bias_rudder = &env->log.base_stage_side_energy_signed_bias_rudder;
    } else {
        return;
    }

    *eps += 1.0f;
    *kills += env->kill ? 1.0f : 0.0f;
    *ground += ground_termination ? 1.0f : 0.0f;
...
void add_log(Dogfight *env) {
    ...
    if (env->stage == mastery_stage) {
        ...
        int ground_termination = env->death_reason == DEATH_OOB
            && (env->player.pos.z < 0.0f || env->opponent.pos.z < 0.0f);
        env->log.base_stage_ground += ground_termination ? 1.0f : 0.0f;
...
void c_step(Dogfight *env) {
    ...
    env->elevator_bias += env->actions[1];
    env->rudder_bias += env->actions[3];
