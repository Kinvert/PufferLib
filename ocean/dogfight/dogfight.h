...
void rlSetClipPlanes(double near_plane, double far_plane);
}
#else
...
struct Env;  // Forward declare Dogfight struct
typedef void (*SpawnFn)(struct Env*, Vec3, Vec3);
...
static void spawn_tail_chase(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_head_on(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_vertical(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_gentle_turns(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_offset(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_angled(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_side(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_dive_attack(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_zoom_attack(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_rear(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_full_predictable(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_full_random(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_medium_turns(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_hard_maneuvering(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_crossing(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_evasive(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_autoace(struct Env *env, Vec3 player_pos, Vec3 player_vel);
// Forced vertical merge spawns (self-play curriculum)
static void spawn_vertical_apex(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_vertical_past(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_vertical_midclimb(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_vertical_merge(struct Env *env, Vec3 player_pos, Vec3 player_vel);
static void spawn_vertical_premerge(struct Env *env, Vec3 player_pos, Vec3 player_vel);
...
typedef struct Env {
    ...
    // Thin aliases retained by the proven Dogfight 3 simulation core.
    float *observations;
    float *actions;
    float *rewards;
    float *terminals;
...
void init(Dogfight *env, int obs_scheme, RewardConfig *rcfg, int curriculum_enabled, int curriculum_randomize, int env_num) {
    ...
    env->recovery_rng_state = dogfight_rand();
...
CurriculumStage get_curriculum_stage(Dogfight *env) {
    ...
    if (env->curriculum_randomize) {
        ...
        return (CurriculumStage)(dogfight_rand() % CURRICULUM_COUNT);
    }
...
#ifdef __CUDACC__
#define float3 dogfight_raymath_float3
#endif
#include "dogfight_render.h"
#ifdef __CUDACC__
#undef float3
#endif
#include "dogfight_puffer.h"
